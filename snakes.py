...
def welcome():
    ...
    while not exit_game :
        #background color
        gamewindow.fill(w_col)
        gamewindow.blit(wlimg,(0,0))
        #title and instructions
        text_screen('Welcome To Snake Game', black ,40,160 )
        text_screen('Press SpaceBar To Play', black , 60,200 )
        for event in pygame.event.get():
            if event.type == pygame.QUIT:
                exit_game=True
            if event.type==pygame.KEYDOWN:
...
def gameloop():
    ...
    while not exit_game:
        if game_over:
            ...
            gamewindow.fill(white)
            gamewindow.blit(gvimg,(0,0))
            text_screen("Press Enter To Continue",red,170,450)
